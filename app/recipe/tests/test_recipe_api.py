...
def detail_url(recipe_id):
    """Create and return a recipe setail url"""
...
class PrivateRecipeAPITests(TestCase):
    ...
    def test_retrieve_recipes(self):
        """Test retrieveing a set of recipes"""
    ...
    def test_recipe_limited_to_user(self):
        """Test lists of recipes limited to authenticated user"""
    ...
    def test_create_recipe_with_new_ingrediants(self):
        ...
        payload = {
            'title': 'Caluiflower tacos',
            'time_minutes': 60,
            'price': Decimal('4.30'),
            'ingrediants': [{'name': 'caulifolwer'}, {'name': 'sault'}],
        ...
        for ingrediant in payload['ingrediants']:
            exist = recipe.ingrediants.filter(
                user=self.user,
    ...
    def test_creating_recipe_with_existing_ingrediant(self):
        """Test creating a recipe with existing ingrediants"""

        ingrediant  = Ingrediant.objects.create(user=self.user, name='lemon')
        payload = {
            'title': 'Vietnamese soup',
            'time_minutes': 25,
            'price': 2.25,
            'ingrediants': [{'name': 'lemon'}, {'name':'fish soup'}]
        }
    ...
    def test_create_ingrediant_on_update(self):
        """Test creating an ingrediant when updating a recipe"""

        recipe = create_Recipe(user=self.user)
        payload = {'ingrediants': [{'name': 'Liam'}]}

        url = detail_url(recipe.id)
        res = self.client.patch(url, payload, format='json')
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        new_ingrediant = Ingrediant.objects.get(user=self.user, name='Liam')
    ...
    def test_update_recipe_assign_ingrediant(self):
        ...
        ingrediant2 = Ingrediant.objects.create(user=self.user, name='Chili')
    ...
    def test_clear_recipe_ingrediants(self):
        """Test clearing a recipes ingrediants """
